...
# Build the run list: (label, method, tokens)
LABELS=(); METHODS=(); TOKENS=()
if [[ "$INCLUDE_NONE" == "1" ]]; then
    LABELS+=("spec_none"); METHODS+=("none"); TOKENS+=("0")
fi
for m in $SPEC_METHODS; do
    for n in $SPEC_TOKENS_LIST; do
        LABELS+=("spec_${m}_${n}"); METHODS+=("$m"); TOKENS+=("$n")
    done
done
...
for r in rows:
    m = re.match(r"spec_(none|dspark|kimi_k3_mtp)(?:_(\d+))?$", r["Config"] or "")
    if not m: continue
    key = (m.group(1), int(m.group(2)) if m.group(2) else 0)
...
for ds in ("sharegpt", "random"):
    ...
    hdr = f"    {'method':13} {'tok':>3} " + " ".join(f"{'c'+c:>18}" for c in concs)
    ...
    for (meth, ntok), v in sorted(present, key=lambda kv: (kv[0][0], kv[0][1])):
        ...
        print(f"    {meth:13} {ntok:>3} " + " ".join(f"{x:>18}" for x in cells))
